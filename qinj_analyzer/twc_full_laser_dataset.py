...
class Painter:
    ...
    def timewalk(self):
        ...
        hh = ax.hist2d(self.frame['tot'], self.frame['toa'], bins=[30,20], range=[[3.0,4.5],[9.5,10.5]], cmin=1)
        ...
        x = np.linspace(self.frame['tot']-0.1, self.frame['tot']+0.1, num=100)
    ...
    def compareTOA(self, popt):
        ...
        for cnt, ifile in enumerate(file_list):
            ...
            ax1.hist(df['toa'], bins=50, range=(9.5, 10.5), histtype='step', label='mean = %.3g\nstd = %.3g'%(mean1,std1))
            ...
            fig1.savefig(self.plot_dir + '/pixel'+ str(self.pixel) + '_laser' + str(q[cnt]) + '_DAC' + str(options.dac) + '_TWC_TOA_fullDataSet.png', dpi=300)
