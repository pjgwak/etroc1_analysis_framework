...
parser.add_option("--charge", help="charge", dest="charge", default="15")
...
def process_data(Q_value, dir_path, DAC_value):
    ...
    for filename in file_list:
        ...
        data.to_csv(sub_file_dir + '/q' + str(Q_value) + '_DAC' + str(dac) + '_time.txt', sep='\t', index=None, header=None)
...
class Painter:
    ...
    def read_data_files(self):
        self.data = pd.read_csv(self.sub_file_dir + '/q' + str(options.charge) + '_DAC'+ str(options.dac) +'_time.txt', sep='\t', header=None)
    ...
    def draw_distribution(self):
        ...
        ax1.hist(self.data['toaCode'], bins=100, range=(130, 230), histtype='step', label='mean = %.3g\nstd = %.3g'%(mean1,std1))
        ax1.set_xlabel('TOA Code', fontsize=13)
        ax1.legend(loc='best')

        ax2.hist(self.data['toa'], bins=75, range=(8.0, 9.5), histtype='step', label='mean = %.3g\nstd = %.3g'%(mean2,std2))
        ax2.set_xlabel('TOA before TWC (ns)', fontsize=13)
        ax2.legend(loc='best')

        fig1.savefig(self.plot_dir + '/pixel'+ str(self.pixel) + '_q' + str(options.charge) + '_DAC' + str(options.dac) + '_TOA.png', dpi=300)
        ...
        ax1.hist(self.data['totCode'], bins=100, range=(0, 100), histtype='step', label='mean = %.3g\nstd = %.3g'%(mean1,std1))
        ax1.set_xlabel('TOT Code', fontsize=13)
        ax1.legend(loc='best')

        ax2.hist(self.data['tot'], bins=100, range=(0.0, 4.0), histtype='step', label='mean = %.3g\nstd = %.3g'%(mean2,std2))
        ax2.set_xlabel('TOT (ns)', fontsize=13)
        ax2.legend(loc='best')

        fig1.savefig(self.plot_dir + '/pixel'+ str(self.pixel) + '_q' + str(options.charge) + '_DAC' + str(options.dac) + '_TOT.png', dpi=300)
        ...
        ax3.legend(loc='best')
        fig2.savefig(self.plot_dir + '/pixel'+ str(self.pixel) + '_q' + str(options.charge) + '_DAC' + str(options.dac) + '_CAL.png', dpi=300)
    ...
    def compareTOA(self):
        ...
        fig1.savefig(self.plot_dir + '/pixel'+ str(self.pixel) + '_q' + str(options.charge) + '_DAC' + str(options.dac) + '_TWC_TOA.png', dpi=300)
...
def main():
    ...
    for charge in [5, 10, 15, 20, 25, 30]:
        if os.path.exists(sub_file_dir + '/q' + str(charge) + '_DAC' + str(options.dac) + '_time.txt'):
            print('File (Q=%s, DAC=%s) already exists' %(str(charge), str(options.dac)))
            continue
